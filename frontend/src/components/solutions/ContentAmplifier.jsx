...
import { 
    ...
    Settings, Award, Mic, AlertTriangle
} from 'lucide-react';
...
const ContentAmplifier = () => {
    ...
    // 2.  TTS FUNCTION 
    const speakText = async (textToSpeak = enhancedText) => {
        ...
        try {
            ...
            if (data.success) {
                console.log('Amplifier TTS successful:', data);
    ...
    // 3. VOICE INPUT FOR CONTENT CREATION
        ...
        try {
            ...
            mediaRecorder.onstop = async () => {
                ...
                if (result.success && result.transcription) {
                    ...
                    // Auto-enhance the transcribed text
                    enhanceTextWithBackend(result.transcription);
    ...
    const transcribeAudio = async (audioBlob) => {
        try {
            ...
            formData.append('audio_file', audioBlob, 'content_creation.wav');
            ...
            if (data.success) {
                console.log('Content transcribed:', data.transcription);
    ...
    // 5. BACKEND-ENHANCED TEXT AMPLIFICATION
    const enhanceTextWithBackend = async (text) => {
        if (!text.trim()) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                updateAnalytics(data.enhanced_text);
                generateSuggestions(data.enhanced_text);
                console.log('Text amplified successfully with backend');
            } else {
                console.error('Backend enhancement failed:', data.error);
                // Fallback to local enhancement
                enhanceTextLocally(text);
            }
        } catch (error) {
    ...
    // 6. LOCAL ENHANCEMENT FALLBACK
    const enhanceTextLocally = (text) => {
        let enhanced = text;
    ...
    // 7. EMERGENCY DETECTION HANDLER
    const handleEmergencyDetection = (transcription) => {
        setEmergencyDetected(true);
        
        // Auto-enhance with emergency context
        setEmotionalTone('empathetic');
        setCulturalContext('kenyan');
        
        // Speak emergency alert
        speakText("Emergency content detected. Enhanced for clarity and impact.");
        
        // Log emergency
        console.log('Emergency content detected:', transcription);
    };

    // 8. SMART SUGGESTIONS WITH BACKEND
    const generateSmartSuggestions = async (text) => {
        try {
            const response = await fetch(`${API_BASE}/api/v1/suggest/content`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    // Update analytics based on enhanced text
        ...
        <div className="content-amplifier-page">
            ...
            {emergencyDetected && (
                <div className="emergency-banner">
                    ...
                    <span>EMERGENCY CONTENT DETECTED - Enhanced for clarity</span>
                    ...
                    <div className="voice-tools-card glass-card">
                        ...
                        <div className="voice-controls">
                            <button 
                                ...
                                onClick={isRecording ? stopSpeechRecognition : startVoiceInput}
                            >
                                <Mic size={20} />
                            ...
                            {isRecording && (
                                <div className="recording-indicator">
                                    ...
                                    <span>Recording... Speak your message</span>
                                </div>
                            )}
                ...
                <div className="main-amplifier-area">
                    ...
                    {isRecording && (
                        <div className="recording-visualizer glass-card">
                            <div className="visualizer-header">
                                <div className="recording-indicator">
                                    <div className="pulse-dot"></div>
                                    <span>Recording your message... Speak clearly</span>
                                </div>
                                <span className="recording-timer">8s</span>
                            </div>
                    ...
                    <div className="input-section glass-card">
                        ...
                        <textarea
                            ...
                            placeholder="Type your message here or use voice input... (Try: 'I need help' or 'Hello, how are you?')"
                        ...
                        <div className="section-header">
                            ...
                            {enhancedText.length > 0 && inputText.length > 0 && (
                                <div className="enhancement-badge">
                                    ...
                                    <span>+{Math.floor(((enhancedText.split(/\s+/).filter(w => w.length > 0).length - inputText.split(/\s+/).filter(w => w.length > 0).length) / Math.max(1, inputText.split(/\s+/).filter(w => w.length > 0).length)) * 100)}% impact</span>
                                </div>
                            )}
                        </div>
