...
const VoicePersonalizer = () => {
  ...
  const speakText = async (text = testText) => {
    if (!text.trim()) return;
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/v1/tts/synthesize`, {
        ...
        body: JSON.stringify({
          text: text,
          voice_id: activeVoice, // Use the selected voice profile
          speaking_rate: voiceSettings.speed / 50, // Convert to backend format
          pitch: (voiceSettings.pitch - 50) / 50, // Convert to backend format
        })
      });
  ...
  // 3. ADD STT FUNCTION FOR VOICE TRAINING
  const transcribeAudioSample = async (audioBlob, phrase) => {
    try {
      const formData = new FormData();
      formData.append('audio_file', audioBlob, 'training_sample.wav');
      ...
      if (data.success) {
        console.log('Training sample transcribed:', data.transcription);
        
        // Compare with expected phrase for training quality
        const accuracy = calculateAccuracy(data.transcription, phrase);
        
        // You can store this accuracy for training progress
        return { 
          success: true, 
  ...
  const calculateAccuracy = (transcribed, expected) => {
    // Simple accuracy calculation - you can make this more sophisticated
    const transcribedWords = transcribed.toLowerCase().split(' ');
    const expectedWords = expected.toLowerCase().split(' ');
    
    const matches = transcribedWords.filter(word => 
      expectedWords.includes(word)
    ).length;
    
    return (matches / Math.max(transcribedWords.length, expectedWords.length)) * 100;
  };

  // 4. UPDATED RECORDING FUNCTION WITH REAL AUDIO RECORDING
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        const result = await transcribeAudioSample(audioBlob, currentPhrase);
        
        if (result.success) {
          // Update training progress based on accuracy
          setTrainingProgress(prev => Math.min(prev + (result.accuracy / 10), 100));
          
          // Move to next phase or finish
          if (recordingPhase < trainingPhrases.length - 1) {
            setRecordingPhase(prev => prev + 1);
            // Continue recording next phrase
      ...
      // Stop recording after 5 seconds per phrase
      setTimeout(() => {
  ...
  // 5. ADD TEXT ENHANCEMENT FUNCTION
  const enhanceTestText = async () => {
    ...
    try {
      ...
      if (data.success) {
        setTestText(data.enhanced_text);
  ...
  const finishRecording = () => {
    ...
    // Stop media recorder if it's still recording
            ...
            <div className="playground-content">
              <div className="test-input">
                ...
                <div className="test-actions">
                  ...
                  {/* 6. ADDED ENHANCEMENT BUTTON */}
                  <button 
                    className="btn-secondary"
                    onClick={enhanceTestText}
